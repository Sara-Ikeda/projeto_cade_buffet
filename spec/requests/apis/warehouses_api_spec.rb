...
describe 'Buffet API' do
  context 'GET /api/v1/buffets' do
    it 'sucesso: listagem completa de buffets cadastrados' do
      ...
      expect(json_response.first["contact"]).to  eq '55961524798 | noivos@contato.com'
      expect(json_response.last["trade_name"]).to  eq 'Doces & Salgados SP'
      expect(json_response.last["contact"]).to  eq '55985943684 | sac@docesesalgados.com'
    end
  ...
  context 'GET /api/v1/buffets?search=value' do
    it 'sucesso: filtra  a lista de buffets pelo nome' do
      owner = Owner.create!(email: 'sara@email.com', password: 'password')
      address = Address.create!(street: 'Av Paulista', number: 50, district: 'Bela Vista',
                  city: 'São Paulo', state: 'SP', zip: '01153000')
      buffet = Buffet.create!(trade_name: 'Gourmet dos Noivos', company_name: 'Buffet Gourmet LTDA',
      ...
      get '/api/v1/buffets?search=Doces'

      expect(response.status).to eq 200
      expect(response.content_type).to include 'application/json'
      json_response = JSON.parse(response.body)
      expect(json_response.first["trade_name"]).to  eq 'Doces & Salgados SP'
      expect(json_response.first["trade_name"]).to_not  eq 'Gourmet dos Noivos'
    end

    it 'sucesso porém não há buffets com o nome buscado' do
      get '/api/v1/buffets?search=Doces'
